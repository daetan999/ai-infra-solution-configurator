"""HTTP response contracts and route registration."""
...
def _download(content: str, *, media_type: str, filename: str, svg: bool = False) -> Response:
    headers = {
...
def _export_response(run: Mapping[str, object], export_format: str) -> Response:
    run_id = int(run["id"])
    if export_format == "json":
        return _download(
            json_export(run), media_type="application/json", filename=f"solution-run-{run_id}.json"
        )
    ...
    return _download(
        validate_svg_export(run.get("architecture_svg")),
...
def _latest_run(repository: object, scenario_id: int) -> Mapping[str, object] | None:
    scenario = repository.get_scenario(scenario_id)  # type: ignore[attr-defined]
...
def register_interface_routes(application: FastAPI) -> None:
...
def register_scenario_collection_routes(application: FastAPI, repository: object) -> None:
    @application.get("/api/scenarios")
    async def list_scenarios() -> JSONResponse:
        scenarios = repository.list_scenarios()  # type: ignore[attr-defined]
        summaries = [
    ...
    async def create_scenario(payload: ScenarioCreate) -> JSONResponse:
        scenario = repository.create_scenario(payload)  # type: ignore[attr-defined]
        return _success(_scenario_bundle(scenario), status_code=201)


def register_scenario_item_routes(application: FastAPI, repository: object) -> None:
    ...
    async def update_scenario(scenario_id: int, payload: ScenarioCreate) -> JSONResponse:
        scenario = repository.update_scenario(scenario_id, payload)  # type: ignore[attr-defined]
        if scenario is None:
            return _error(404, "not_found", "Scenario was not found.")
        return _success(_scenario_bundle(scenario))
    ...
    async def delete_scenario(scenario_id: int) -> JSONResponse:
        if not repository.delete_scenario(scenario_id):  # type: ignore[attr-defined]
            return _error(404, "not_found", "Scenario was not found.")
        return _success({"deleted": True, "id": scenario_id})


def register_run_read_routes(application: FastAPI, repository: object) -> None:
    @application.get("/api/scenarios/{scenario_id}/runs")
    async def list_runs(scenario_id: int) -> JSONResponse:
        scenario = repository.get_scenario(scenario_id)  # type: ignore[attr-defined]
        if scenario is None:
            return _error(404, "not_found", "Scenario was not found.")
        runs = repository.list_runs(scenario_id)  # type: ignore[attr-defined]
    ...
    async def get_run(run_id: int) -> JSONResponse:
        run = repository.get_run(run_id)  # type: ignore[attr-defined]
        if run is None:
            return _error(404, "not_found", "Assessment run was not found.")
        return _success({"run": _public_run(run)})


def register_run_export_routes(application: FastAPI, repository: object) -> None:
    def require_run(run_id: int) -> Mapping[str, object] | None:
        return repository.get_run(run_id)  # type: ignore[attr-defined]

    @application.get("/api/runs/{run_id}/export.json")
    async def export_run_json(run_id: int) -> Response:
        run = require_run(run_id)
        return (
            _error(404, "not_found", "Assessment run was not found.")
            if run is None
            else _export_response(run, "json")
        )
    ...
    async def export_run_markdown(run_id: int) -> Response:
        ...
        return (
            _error(404, "not_found", "Assessment run was not found.")
            if run is None
            else _export_response(run, "markdown")
        )
    ...
    async def export_run_svg(run_id: int) -> Response:
        ...
        return (
            _error(404, "not_found", "Assessment run was not found.")
            if run is None
            else _export_response(run, "svg")
        )


def register_scenario_export_routes(application: FastAPI, repository: object) -> None:
    @application.get("/api/scenarios/{scenario_id}/diagram.svg")
    async def scenario_diagram(scenario_id: int) -> Response:
        run = _latest_run(repository, scenario_id)
        return (
            _error(404, "not_found", "Scenario was not found.")
            if run is None
            else _export_response(run, "svg")
        )
    ...
    async def scenario_export(
        ...
        return (
            _error(404, "not_found", "Scenario was not found.")
            if run is None
            else _export_response(run, format)
        )


def register_routes(application: FastAPI, repository: object) -> None:
    register_exception_handlers(application)
    register_interface_routes(application)
    register_scenario_collection_routes(application, repository)
    register_scenario_item_routes(application, repository)
    register_run_read_routes(application, repository)
    register_run_export_routes(application, repository)
    register_scenario_export_routes(application, repository)
