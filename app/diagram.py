...
_NODE_LAYOUT: Final[dict[str, _NodeBox]] = {
    "applications": _NodeBox(52, 252, 190, 106),
    "api-gateway": _NodeBox(292, 252, 190, 106),
    "model-serving": _NodeBox(532, 178, 200, 112),
    "runtime": _NodeBox(782, 178, 200, 112),
    "gpu-compute": _NodeBox(1032, 178, 220, 112),
    "retrieval": _NodeBox(532, 402, 200, 112),
    "data-platform": _NodeBox(782, 402, 200, 112),
    "object-storage": _NodeBox(1032, 402, 220, 112),
    "network": _NodeBox(112, 650, 300, 102),
    "security": _NodeBox(570, 650, 300, 102),
    "observability": _NodeBox(1028, 650, 300, 102),
}
_NODE_COLORS: Final[dict[NodeKind, tuple[str, str, str]]] = {
...
_EDGE_COLORS: Final[dict[EdgeKind, str]] = {
...
def _render_edge(edge: ArchitectureEdge) -> str:
    ...
    return "\n".join(
        (
            f'    <g data-edge-kind="{edge.kind.value}">',
            f'      <path d="{path}" fill="none" stroke="{color}" stroke-width="2"{dash} marker-end="url(#arrow-{edge.kind.value})"/>',
            f'      <rect x="{label_x - 44}" y="{label_y - 12}" width="88" height="22" rx="11" fill="#FCFDFC" stroke="#DCE4E1"/>',
            f'      <text x="{label_x}" y="{label_y + 4}" text-anchor="middle" font-size="11" font-weight="600" fill="#4B625D">{_xml_text(edge.label.upper())}</text>',
            "    </g>",
        )
    )
...
def _render_node(node: ArchitectureNode) -> str:
    ...
    detail = _wrap_label(node.detail, width=34, lines=1)[0]
    return "\n".join(
        (
            f'    <g data-node-id="{node.id}" data-layer="{node.layer.value}" data-node-kind="{node.kind.value}">',
            f'      <rect x="{box.x}" y="{box.y}" width="{box.width}" height="{box.height}" rx="14" fill="#0C211F" opacity="0.08" transform="translate(0 4)"/>',
            f'      <rect x="{box.x}" y="{box.y}" width="{box.width}" height="{box.height}" rx="14" fill="{fill}" stroke="{stroke}" stroke-width="1.5"/>',
            f'      <rect x="{box.x}" y="{box.y}" width="6" height="{box.height}" rx="3" fill="{stroke}"/>',
            f'      <text font-size="16" font-weight="700" fill="{text_color}">{tspans}</text>',
            f'      <text x="{box.x + 18}" y="{box.y + box.height - 18}" font-size="11" fill="#5C706C">{_xml_text(detail)}</text>',
            "    </g>",
        )
    )
...
def render_architecture_svg(blueprint: ArchitectureBlueprint) -> str:
    ...
    markers = "\n".join(
        f'      <marker id="arrow-{kind.value}" markerWidth="9" markerHeight="9" refX="8" refY="4.5" orient="auto"><path d="M 0 0 L 9 4.5 L 0 9 z" fill="{_EDGE_COLORS[kind]}"/></marker>'
        for kind in EdgeKind
    )

    return "\n".join(
        (
            f'<svg xmlns="http://www.w3.org/2000/svg" width="{_CANVAS_WIDTH}" height="{_CANVAS_HEIGHT}" viewBox="0 0 {_CANVAS_WIDTH} {_CANVAS_HEIGHT}" role="img" aria-labelledby="architecture-title architecture-description" font-family="-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif">',
            f'  <title id="architecture-title">{title}</title>',
            f'  <desc id="architecture-description">{description} Initial solution hypothesis; requires technical validation.</desc>',
            "  <defs>",
            markers,
            "  </defs>",
            ...
            '  <text x="58" y="76" font-size="12" font-weight="700" letter-spacing="1.8" fill="#6D817C">ENTERPRISE AI SOLUTION CONFIGURATOR</text>',
            ...
            '  <text x="58" y="203" font-size="11" font-weight="700" letter-spacing="1.2" fill="#85958F">EXPERIENCE</text>',
            '  <text x="298" y="203" font-size="11" font-weight="700" letter-spacing="1.2" fill="#85958F">INGRESS</text>',
            '  <text x="538" y="203" font-size="11" font-weight="700" letter-spacing="1.2" fill="#85958F">AI SERVICES</text>',
            '  <text x="788" y="203" font-size="11" font-weight="700" letter-spacing="1.2" fill="#85958F">PLATFORM &amp; DATA</text>',
            '  <text x="1038" y="203" font-size="11" font-weight="700" letter-spacing="1.2" fill="#85958F">COMPUTE &amp; STORAGE</text>',
            '  <rect x="58" y="610" width="1324" height="174" rx="18" fill="#FBFAF6" stroke="#E8E2CF"/>',
            '  <text x="80" y="635" font-size="11" font-weight="700" letter-spacing="1.2" fill="#8A7B47">CROSS-CUTTING CONTROLS</text>',
            ...
            '  <text x="720" y="838" text-anchor="middle" font-size="12" font-weight="700" letter-spacing="0.8" fill="#315A53">INITIAL SOLUTION HYPOTHESIS · REQUIRES TECHNICAL VALIDATION</text>',
            "</svg>",
        )
    )
